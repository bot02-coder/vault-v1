...
export default function AdminPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    setLoading(false);
    if (res.ok) alert('Success! Post sent to @hi0anime');
    else alert('Error! Check your password or Vercel logs.');
  };
    ...
    <div style={{backgroundColor: '#000', color: '#fff', minHeight: '100vh', padding: '40px', fontFamily: 'sans-serif'}}>
      <div style={{maxWidth: '500px', margin: '0 auto', border: '1px solid #333', padding: '20px', borderRadius: '10px'}}>
        ...
        <form onSubmit={handleSubmit} style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
          ...
          <textarea style={{padding: '10px', background: '#111', border: '1px solid #444', color: '#fff', height: '100px'}} placeholder="Summary" onChange={e => setForm({...form, description: e.target.value})} required />
          <input style={{padding: '10px', background: '#111', border: '1px solid #444', color: '#fff'}} placeholder="Tags (fantasy, romance)" onChange={e => setForm({...form, tags: e.target.value})} required />
          <input style={{padding: '10px', background: '#111', border: '1px solid #444', color: '#fff'}} placeholder="Cover Image URL" onChange={e => setForm({...form, coverUrl: e.target.value})} required />
          <input style={{padding: '10px', background: '#111', border: '1px solid #444', color: '#fff'}} placeholder="Manga Link (Telegraph)" onChange={e => setForm({...form, destUrl: e.target.value})} required />
